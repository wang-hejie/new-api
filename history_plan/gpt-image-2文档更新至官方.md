...
- [ ] 0.1 已读完 4 个 docs 文件、`gpt-image-2 openai官方支持参数清单.md`、两份 history_plan。
- [ ] 0.2 已确认 `relay/channel/openai/adaptor.go` 中 `openaiGPTImage2EditPassthrough` 白名单为 `{prompt, n, size, quality, user, background, moderation, output_format, output_compression}`，并由 `mf.File` 路径独立处理 `image`、`image[]`、`mask`，与本轮文档保留字段不冲突。
- [ ] 0.3 已确认本轮不需要触碰 `relay/`、`dto/`、`controller/`、前端 / E2E 代码；范围仅限 `docs/guides/`。
- [ ] 0.4 已记录所有"待删除字段 / 字符串字面量"清单，供阶段 6 残留扫描验证：
  - `response_format`
  - `reference_usage`
  - `aspect_ratio`
...
- [ ] 1.1 改后该文件全文 `grep -nE 'response_format|aspect_ratio|background|reference_usage' docs/guides/gpt-image-2.md` 无任何命中。
- [ ] 1.2 文档语义完整：标题层级、章节计数、计费规则与错误处理段落未被牵连。
- [ ] 1.3 文档不含"修复 / 历史 / 旧 / 已删除 / 兼容"等说明文字。
...
- [ ] 2.1 `grep -nE 'response_format|aspect_ratio|b64_json 与|url 两种' docs/guides/gpt-image-2-generations.md` 仅允许命中响应样例中的 `b64_json`（字段值），不允许命中 `response_format` 任意位置。
- [ ] 2.2 Request Body Example 仍是合法 JSON（必要时用 `jq` 验证）。
- [ ] 2.3 Body 表恰好 5 行（`model / prompt / n / size / quality`）。
- [ ] 2.4 章节顺序：`Authorizations → POST → Request → Body → Request Body Example → Response → Tested Results → Error Responses → Usage Notes`。
...
- 锚点 D：Request Example 中 `-F "reference_usage=subject" \` → 整行删除；上一行 `-F "quality=low" \` 末尾的反斜杠保留不变（curl 续行）。
- 锚点 E：Response 样例中 `,\n  "reference_blobs": []` → 删除该行并把前一行 `"account": "..."` 末尾的 `,` 删掉。
- 锚点 F：Tested Result 表行中的 `+ \`reference_usage=subject\`` 子串 → 删除（连同前置空格与 `+`）。

### 6.3 自检

- [ ] 3.1 `grep -nE 'response_format|reference_usage|reference_blobs' docs/guides/gpt-image-2-edits.md` 无任何命中。
- [ ] 3.2 Body 表恰好 6 行（`image / prompt / model / n / size / quality`）。
- [ ] 3.3 curl Request Example 仍能在 shell 解析（反斜杠续行不残缺）；可以用 `bash -n` 解析含 heredoc 的复制版本验证语法。
- [ ] 3.4 Response 样例仍是合法 JSON（用 `jq` 验证）。
...
- [ ] 4.1 `grep -nE 'response_format|reference_usage|extra_body|显式返回 URL|返回 URL' docs/guides/gpt-image-2-examples.md` 无任何命中。
- [ ] 4.2 Python 代码块用 `python -m py_compile <(...)` 或临时落盘后 `python -m py_compile` 验证语法。
- [ ] 4.3 curl 代码块的反斜杠续行无残缺；可临时落盘后 `bash -n` 验证。
- [ ] 4.4 文档结构：`Python OpenAI SDK → 基础生图 → 参考图编辑 → Node.js OpenAI SDK → curl → 文本生成图像 → 参考图编辑`，无遗留小标题。

---

## 八、阶段 5 — 静态校对

- [ ] 5.1 4 个文档 frontmatter（`slug / title / order / category`）保留不动；用 `grep -n '^---$' docs/guides/gpt-image-2*.md` 应见每文件两条分隔线。
- [ ] 5.2 Markdown 链接、代码块语言标签未被牵连：
  ```bash
  ...
  ```
  代码块开/闭对数应为偶数。
- [ ] 5.3 表头 / 表体列数一致；尤其阶段 2 的 Body 表与阶段 3 的 Body 表删除行后，分隔行 `|---|---|...|` 列数不能变。
- [ ] 5.4 若项目对 docs 有 Mintlify / 自定义渲染流程（参考 `993a2969` 提交"将 doc 全部页面改造为 evolink Mintlify 风格"），可在本地用相应预览命令打开 4 个页面看一眼渲染结果；该校验为可选项，不阻塞计划完成。

---

## 九、阶段 6 — 残留扫描 + 完结

- [ ] 6.1 全局残留扫描（仅扫 docs 目录，避免误命中代码 / history_plan）：
  ```bash
...
- [ ] 6.2 反向语义扫描，确保未误删保留字段：
  ...
  ```
  期望：保留字段在 Body 表中可见。
- [ ] 6.3 全局变更解释残留扫描，确保最终文档不含"修复 / 旧接口 / 新接口 / 已删除 / 已废弃 / 历史 / 兼容性 / 变更 / 不再支持"等中间过程描述：
  ```bash
...
- [ ] 6.5 不要在 docs 中新增 `output_format / output_compression / background / moderation / user / mask / image[]` 等字段（这些字段的文档化属于下一轮"能力扩展"工作，本轮不动）。
- [ ] 6.6 提交时按 CLAUDE.md 工作流：commit message 中文、不动受保护品牌信息、不批量删 i18n locale。
- [ ] 6.7 把"Review" 区补完（见第十二节）。
...
- 实际修改文件清单：
  - [ ] `docs/guides/gpt-image-2.md`
  - [ ] `docs/guides/gpt-image-2-generations.md`
  - [ ] `docs/guides/gpt-image-2-edits.md`
  - [ ] `docs/guides/gpt-image-2-examples.md`
- 校验结果：
  - [ ] 阶段 6.1 残留扫描无命中
  - [ ] 阶段 6.2 保留字段扫描通过
  - [ ] 阶段 6.3 变更解释残留扫描无命中
